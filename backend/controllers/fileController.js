...
import {uploadBase64ToGCS, uploadFileByFile} from '../services/googleService.js'
...
export const uploadFile = catchAsyncError(async (req, res) => {
  ...
  const result = await uploadBase64ToGCS(file.buffer.toString('base64'), file.originalname.replaceAll(" ",'-'), file.mimetype)
...
export const uploadFileStructure = catchAsyncError(async (req, res) => {
  ...
  const processFolderData = async (data, parentFolderId = null) => {
    if (data.type === 'folder') {
      ...
      //   resource_type: 'auto',
      // });
      const result = await uploadBase64ToGCS(data.file.data, data.file.name.replaceAll(" ",'-'), data.file.mimeType)
